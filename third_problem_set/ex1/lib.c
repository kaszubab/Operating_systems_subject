...
// mode - specifies whether atime or mtime was used
// 0 - neither, 1 - atime, 2 - mtime
// sgn 1 -> +  sgn -1 -> - with atime/mtime option, 0 -> not speified 
// n -> number of days
...
void search_by_stat(char * dir_path, int depth, int mode, int sgn, int days)
{
    ...
    while( (file_pointer = readdir(directory)) != NULL)
    {
        ...
        if (file_pointer->d_type == 4)
        {
            pid_t child_process = fork();

            if (child_process == 0) //if this is a child
            {
                ...
                else if(ls_id < 0)
                {
                    ...
                    waitpid(ls_id, NULL, 0);
                    search_by_stat(next_path, depth-1, mode, sgn, days);
                }
            }
