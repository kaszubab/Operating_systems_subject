...
void say(int size, int prepare, int send)
{
    ...
    printf("(%d %ld %ldms) Wysłałem zamówienie o wielkości : %d. Liczba zamównień do przygotowania: %d. Liczba zamównień do wysłania: %d. \n", getpid(), tm.tv_sec, tm.tv_nsec%1000, size, prepare, send);
}
...
int main(int argc, char ** argv)
{
    ...
    while(TRUE)
    {
        ...
        int order_size = orders->orders[orders->send_order_index];
        orders->send_order_index = (orders->send_order_index + 1) % MAXIMUM_ORDERS;
        
        int prepare = semctl(sem_id, 2, GETVAL);
        int send = semctl(sem_id, 3, GETVAL);


        say(order_size *3, prepare, send+1);
